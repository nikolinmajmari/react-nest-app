...
export default function NewChannelModal(props:ICreateChannelModalProps){
    ...
    const back = useCallback(
        ()=>navigate(location.state?.previousLocation ?? "/chat/channels"),[navigate,location.state]
    );
...
export function UserSearchPicker({selected,setSelected,results,search,setSearch,error,multiple}:IUserSearchPcikerProps){
    ...
    return (
        <div className="flex flex-col items-stretch justify-start">
            <SearchComponent value={search} onChange={(e)=>setSearch(e.target.value)} type="search" id="default-search"  placeholder="Search Users" />
            <SelectedUsers toggleUser={toggleSelectUser} users={selected}/>
            <div className="results flex flex-col overflow-y-auto mb-4 h-64">
                {
                    results.map(
                        user=><SearchedUserItem key={user.id} 
                                user={user} 
